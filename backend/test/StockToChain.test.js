...
describe("StockToChain Contract", function () {
    async function deployStockToChainFixture() {
        ...
        const StockToChain = await ethers.getContractFactory("StockToChain");
        const stockToChain = await StockToChain.deploy(
            await eurUsdPriceFeed.getAddress(),
            await polUsdPriceFeed.getAddress(),
            companyWallet.address,
            platformWallet.address
        );
        await stockToChain.waitForDeployment();
    ...
    beforeEach(async function () {
        ({ stockToChain, owner, addr1, addr2, addr3, companyWallet, platformWallet, eurUsdPriceFeed, polUsdPriceFeed } = await loadFixture(deployStockToChainFixture));
    });

    describe("Deployment", function () {
        it("Should set the right owner", async function () {
            expect(await stockToChain.owner()).to.equal(companyWallet.address);
        });

        it("Should set the correct workflow status", async function () {
            expect(await stockToChain.workflowStatus()).to.equal(0);
        });

        it("Should set the correct platform wallet", async function () {
            expect(await stockToChain.platformWallet()).to.equal(platformWallet.address);
        });
    });

    describe("Whitelist Management", function () {
        it("Should allow owner to add addresses to whitelist", async function () {
            await expect(stockToChain.connect(companyWallet).addToWhitelist([addr1.address]))
                .to.emit(stockToChain, "InvestorWhitelisted").withArgs(addr1.address);
            expect((await stockToChain.investors(addr1.address)).whitelisted).to.be.true;
        });

        it("Should not allow non-owner to add addresses to whitelist", async function () {
            await expect(stockToChain.connect(addr1).addToWhitelist([addr2.address]))
                .to.be.revertedWithCustomError(stockToChain, "OwnableUnauthorizedAccount");
        });

        it("Should allow adding multiple addresses to whitelist", async function () {
            await stockToChain.connect(companyWallet).addToWhitelist([addr1.address, addr2.address]);
            expect((await stockToChain.investors(addr1.address)).whitelisted).to.be.true;
            expect((await stockToChain.investors(addr2.address)).whitelisted).to.be.true;
        });
        ...
        it("Should allow owner to remove addresses from whitelist", async function () {
            ...
            await expect(stockToChain.connect(companyWallet).removeFromWhitelist([addr1.address, addr2.address]))
                .to.emit(stockToChain, "InvestorRemovedFromWhitelist")
                .withArgs(addr1.address)
                .to.emit(stockToChain, "InvestorRemovedFromWhitelist")
                .withArgs(addr2.address);

            // Vérification que les investisseurs ont été retirés de la whitelist
            expect((await stockToChain.investors(addr1.address)).whitelisted).to.be.false;
            expect((await stockToChain.investors(addr2.address)).whitelisted).to.be.false;
        });

        it("Should not allow non-owner to remove addresses from whitelist", async function () {
            await expect(stockToChain.connect(addr1).removeFromWhitelist([addr2.address]))
                .to.be.revertedWithCustomError(stockToChain, "OwnableUnauthorizedAccount");
        });
    });
    ...
    describe("Token Sale", function () {
        beforeEach(async function () {
            await stockToChain.connect(companyWallet).addToWhitelist([addr1.address]);
            await stockToChain.connect(companyWallet).updateWorkflowStatus(1);
        });

        it("Should allow whitelisted users to buy tokens", async function () {
            const amount = ethers.parseEther("1");
            const price = await stockToChain.getCurrentPriceInPOL(amount);
            await expect(stockToChain.connect(addr1).buyTokens(amount, { value: price }))
                .to.emit(stockToChain, "TokensPurchased").withArgs(addr1.address, amount, price);
        });

        it("Should not allow non-whitelisted users to buy tokens", async function () {
            const amount = ethers.parseEther("1");
            const price = await stockToChain.getCurrentPriceInPOL(amount);
            await expect(stockToChain.connect(addr2).buyTokens(amount, { value: price }))
                .to.be.revertedWith("Address not whitelisted");
        });

        it("Should not allow buying tokens when sale is not active", async function () {
            await stockToChain.connect(companyWallet).updateWorkflowStatus(2);
            const amount = ethers.parseEther("1");
            const price = await stockToChain.getCurrentPriceInPOL(amount);
            await expect(stockToChain.connect(addr1).buyTokens(amount, { value: price }))
        ...
        it("Should revert if amount is zero", async function () {
            await expect(stockToChain.connect(addr1).buyTokens(0, { value: 0 }))
        ...
        it("Should revert if not enough POL sent", async function () {
            const amount = ethers.parseEther("1");
            const price = await stockToChain.getCurrentPriceInPOL(amount);
            await expect(stockToChain.connect(addr1).buyTokens(amount, { value: price - 1n }))
        ...
        it("Should not add investor twice to investorList", async function () {
            ...
            // Premier achat
            await stockToChain.connect(addr1).buyTokens(amount, { value: price });
            
            // Deuxième achat
            await stockToChain.connect(addr1).buyTokens(amount, { value: price });

            // Vérification que l'investisseur n'est pas dupliqué
            const investor = await stockToChain.investors(addr1.address);
        ...
        it("Should not refund if exact payment is sent", async function () {
            ...
            const tx = await stockToChain.connect(addr1).buyTokens(amount, { value: price });
    ...
    describe("Profit Distribution", function () {
        ...
        it("Should allow owner to distribute profits", async function () {
            ...
            await expect(stockToChain.connect(companyWallet).distributeProfits({ value }))
                .to.emit(stockToChain, "ProfitsDistributed").withArgs(value);
        });

        it("Should not allow non-owner to distribute profits", async function () {
            const value = ethers.parseEther("1");
            await expect(stockToChain.connect(addr1).distributeProfits({ value }))
                .to.be.revertedWithCustomError(stockToChain, "OwnableUnauthorizedAccount");
        });

        it("Should not allow distributing zero profits", async function () {
            await expect(stockToChain.connect(companyWallet).distributeProfits({ value: 0 }))
    ...
    describe("Profit Distribution - Edge Cases", function () {
        async function setupContractWithNoTokenInCirculation() {
            await stockToChain.connect(companyWallet).updateWorkflowStatus(1); // SaleActive
        }
        ...
        async function setupContractWithInvestorButZeroTokens() {
            ...
            const price = await stockToChain.getCurrentPriceInPOL(amount);
            await stockToChain.connect(addr1).buyTokens(amount, { value: price });
            await stockToChain.connect(addr1).transfer(addr2.address, amount);
        }
        ...
        it("Should revert when no tokens are in circulation", async function () {
            ...
            await expect(
                stockToChain.connect(companyWallet).distributeProfits({ value: profitAmount })
        ...
        it("Should distribute profits correctly when one investor holds tokens", async function () {
            ...
            await expect(
                stockToChain.connect(companyWallet).distributeProfits({ value: profitAmount })
            ).to.emit(stockToChain, "ProfitsDistributed").withArgs(profitAmount);
        ...
        it("Should revert if profit amount is zero", async function () {
            await stockToChain.connect(companyWallet).updateWorkflowStatus(1);
            await expect(
                stockToChain.connect(companyWallet).distributeProfits({ value: 0 })
            ).to.be.revertedWith("No profits to distribute");
        });
    });
    ...
    describe("Profit Claims", function () {
        it("Should not allow claiming profits before waiting period", async function () {
            ...
            const profitAmount = ethers.parseEther("1");
            await stockToChain.connect(companyWallet).distributeProfits({ value: profitAmount });
            ...
            await expect(stockToChain.connect(addr1).claimProfits())
        ...
        it("Should allow claiming profits after waiting period", async function () {
            ...
            const share = await stockToChain.calculateInvestorProfitShare(addr1.address);
            console.log("Profit share after 4 years:", ethers.formatEther(share));
            expect(share).to.be.gt(0);

            await expect(stockToChain.connect(addr1).claimProfits())
                .to.emit(stockToChain, "ProfitsClaimed")
        ...
        it("Should return 0 profit share for investor with no tokens", async function () {
            ...
            await stockToChain.connect(companyWallet).addToWhitelist([addr2.address]);
            const amount = ethers.parseEther("1");
            const price = await stockToChain.getCurrentPriceInPOL(amount);
            await stockToChain.connect(addr2).buyTokens(amount, { value: price });
    ...
    describe("Buyback", function () {
        it("Should not allow buyback when not in buyback phase", async function () {
            ...
            await stockToChain.connect(addr1).buyTokens(amount, { value: price });
            await stockToChain.connect(companyWallet).updateWorkflowStatus(2);
            const buybackPrice = await stockToChain.getBuybackPriceInPOL(amount);
            await expect(stockToChain.connect(companyWallet).buybackByCompanyBatch(0, 1, { value: buybackPrice }))
    ...
    describe("Buyback (Batch)", function () {
        beforeEach(async function () {
            ...
            // Achat de tokens
            const amount = ethers.parseEther("1");
            const price1 = await stockToChain.getCurrentPriceInPOL(amount);
            await stockToChain.connect(addr1).buyTokens(amount, { value: price1 });
            const price2 = await stockToChain.getCurrentPriceInPOL(amount);
            await stockToChain.connect(addr2).buyTokens(amount, { value: price2 });

            // Transfert des tokens au contrat
            await stockToChain.connect(addr1).transfer(await stockToChain.getAddress(), amount);
            await stockToChain.connect(addr2).transfer(await stockToChain.getAddress(), amount);

            // Transition vers BuybackActive en passant par SaleEnded
            await stockToChain.connect(companyWallet).updateWorkflowStatus(2); // SaleEnded
            await stockToChain.connect(companyWallet).updateWorkflowStatus(3); // BuybackActive
        });

        it("Should execute buyback in 2 batches and burn tokens", async function () {
            const totalTokens = await stockToChain.totalSupply();
            console.log("Total tokens before buyback:", ethers.formatEther(totalTokens));
            
            const buybackPrice = await stockToChain.getBuybackPriceInPOL(totalTokens);
            const extra = ethers.parseEther("0.1");
            const totalFunds = buybackPrice + extra;
            const fundsPerBatch = totalFunds / 2n;

            // Batch 1: addr1
            await stockToChain.connect(companyWallet).buybackByCompanyBatch(0, 1, { value: fundsPerBatch });

            // Batch 2: addr2 + final burn
            await expect(stockToChain.connect(companyWallet).buybackByCompanyBatch(1, 2, { value: fundsPerBatch }))
                .to.emit(stockToChain, "BuybackExecuted");

            // Vérifie que tous les tokens sont brûlés
            expect(await stockToChain.totalSupply()).to.equal(0);
        });

        it("Should revert batch if out-of-range indexes are used", async function () {
            const amount = await stockToChain.getBuybackPriceInPOL(await stockToChain.totalSupply());
            await expect(
                stockToChain.connect(companyWallet).buybackByCompanyBatch(0, 5, { value: amount })
        ...
        it("Should revert if buyback batch is called without enough funds in first batch", async function () {
            const price = (await stockToChain.getBuybackPriceInPOL(await stockToChain.totalSupply())) - 1n;
            await expect(
                stockToChain.connect(companyWallet).buybackByCompanyBatch(0, 2, { value: price })
        ...
        it("Should revert if not BuybackActive", async function () {
            ...
            const { stockToChain: newStockToChain } = await loadFixture(deployStockToChainFixture);
            const amount = ethers.parseEther("1");
            await expect(newStockToChain.connect(companyWallet).buybackByCompanyBatch(0, 1, { value: amount }))
                .to.be.revertedWith("Buyback not active");
        });

        it("Should skip initial price calculation when start != 0 in buyback batch", async function () {
            const totalPrice = await stockToChain.getBuybackPriceInPOL(await stockToChain.totalSupply());
            const extra = ethers.parseEther("0.1");

            // Premier batch pour initialiser le buybackPriceFinal
            await stockToChain.connect(companyWallet).buybackByCompanyBatch(0, 1, { value: totalPrice + extra });

            // Deuxième batch avec start != 0
            await expect(stockToChain.connect(companyWallet).buybackByCompanyBatch(1, 2, { value: ethers.parseEther("0.01") }))
                .to.emit(stockToChain, "BuybackExecuted");
        });
    });
    ...
    describe("Additional Coverage Tests", function () {
        it("Should refund excess MATIC after token purchase", async function () {
            ...
            const amount = ethers.parseEther("1");
            const priceInPol = await stockToChain.getCurrentPriceInPOL(amount);
            const excessAmount = ethers.parseEther("0.1");
            const totalAmount = priceInPol + excessAmount;

            const balanceBefore = await ethers.provider.getBalance(addr1.address);
            await stockToChain.connect(addr1).buyTokens(amount, { value: totalAmount });
            const balanceAfter = await ethers.provider.getBalance(addr1.address);
        ...
        it("Should revert buyback if no investors have tokens", async function () {
            ...
            const buybackPrice = await stockToChain.getBuybackPriceInPOL(amount);
            await expect(stockToChain.connect(companyWallet).buybackByCompanyBatch(0, 1, { value: buybackPrice }))
            .to.be.revertedWithCustomError(stockToChain, "NoTokensToBuyBack");
        ...
        it("Should allow emergencyWithdraw of ERC20 token", async function () {
            ...
            await mockToken.transfer(await stockToChain.getAddress(), amount);

            const balanceBefore = await mockToken.balanceOf(companyWallet.address);
            await stockToChain.connect(companyWallet).emergencyWithdraw(await mockToken.getAddress());
        ...
        it("Should allow emergencyWithdraw of MATIC", async function () {
            ...
            await companyWallet.sendTransaction({
                to: await stockToChain.getAddress(),
                value: amount
            });

            const balanceBefore = await ethers.provider.getBalance(companyWallet.address);
            await stockToChain.connect(companyWallet).emergencyWithdraw(ethers.ZeroAddress);
        ...
        it("Should allow the owner to unpause the contract", async function () {
            ...
            await stockToChain.connect(companyWallet).pause();
        
            // Vérifie qu'il est bien en pause
            expect(await stockToChain.paused()).to.be.true;
        
            // Puis on le déverrouille
            await stockToChain.connect(companyWallet).unpause();
        
            // Vérifie qu'il est bien déverrouillé
            expect(await stockToChain.paused()).to.be.false;
        });
        ...
        it("Should return correct investment summary", async function () {
            ...
            const summary = await stockToChain.getInvestmentSummary(addr1.address);
            const investor = await stockToChain.investors(addr1.address);
        ...
        it("Should handle buyback when startIndex equals endIndex", async function () {
            ...
            const buybackPrice = await stockToChain.getBuybackPriceInPOL(amount);
            await expect(stockToChain.connect(companyWallet).buybackByCompanyBatch(0, 0, { value: buybackPrice }))
                .to.be.revertedWith("Invalid index range");
        });
        ...
        it("Should handle distributeProfits when investors count equals BATCH_SIZE", async function () {
            ...
            // Distribution des profits
            const profitAmount = ethers.parseEther("1");
            await expect(stockToChain.connect(companyWallet).distributeProfits({ value: profitAmount }))
                .to.emit(stockToChain, "ProfitsDistributed")
        ...
        it("Should handle distributeProfits when investor has no tokens", async function () {
            ...
            const investor2 = await stockToChain.investors(addr2.address);
        ...
        it("Should call fallback and emit FallbackCalled event", async function () {
            ...
            await expect(tx).to.emit(stockToChain, "FallbackCalled")
    ...
    describe("Workflow Management", function () {
        it("Should revert on invalid workflow transition: SaleNotStarted to SaleEnded", async function () {
            await expect(
                stockToChain.connect(companyWallet).updateWorkflowStatus(2) // SaleEnded
        ...
        it("Should revert on invalid workflow transition: SaleActive to BuybackActive", async function () {
            await stockToChain.connect(companyWallet).updateWorkflowStatus(1); // SaleActive
            await expect(
                stockToChain.connect(companyWallet).updateWorkflowStatus(3) // BuybackActive
            ).to.be.revertedWith("Invalid workflow transition");
        });
        it("Should allow valid transition from SaleActive to SaleEnded", async function () {
            ...
            await expect(
                stockToChain.connect(companyWallet).updateWorkflowStatus(2)    // SaleEnded
            ).to.emit(stockToChain, "WorkflowStatusChanged").withArgs(2);
        });
    ...
    describe("Emergency Withdraw", function () {
        it("Should revert emergencyWithdraw if not owner", async function () {
            await expect(stockToChain.connect(addr1).emergencyWithdraw(ethers.ZeroAddress))
                .to.be.revertedWithCustomError(stockToChain, "OwnableUnauthorizedAccount");
        });
    ...
    describe("Pause Management", function () {
        it("Should revert pause if not owner", async function () {
            await expect(stockToChain.connect(addr1).pause())
                .to.be.revertedWithCustomError(stockToChain, "OwnableUnauthorizedAccount");
        });

        it("Should revert unpause if not owner", async function () {
            await expect(stockToChain.connect(addr1).unpause())
                .to.be.revertedWithCustomError(stockToChain, "OwnableUnauthorizedAccount");
        });
    ...
    describe("Coverage Improvements", function () {
        it("Should skip investor with zero tokens in distributeProfits", async function () {
            ...
            await expect(stockToChain.connect(companyWallet).distributeProfits({ value: profit }))
                .to.emit(stockToChain, "ProfitsDistributed");
        ...
        it("Should skip investor share transfer if share is zero", async function () {
            ...
            // Distribution des profits
            const profit = ethers.parseEther("1");
            await stockToChain.connect(companyWallet).distributeProfits({ value: profit });
        ...
        it("Should revert on invalid workflow transition from BuybackActive to SaleNotStarted", async function () {
            await stockToChain.connect(companyWallet).updateWorkflowStatus(1);
            await stockToChain.connect(companyWallet).updateWorkflowStatus(2);
            await stockToChain.connect(companyWallet).updateWorkflowStatus(3);
            await expect(stockToChain.connect(companyWallet).updateWorkflowStatus(0))
                .to.be.revertedWith("Invalid workflow transition");
        });
        ...
        it("Should revert if oracle returns invalid price", async function () {
            ...
            const amount = ethers.parseEther("1");
            await expect(stockToChain.getCurrentPriceInPOL(amount))
        ...
        it("Should revert emergencyWithdraw if token is the contract token itself", async function () {
            const contractAddress = await stockToChain.getAddress();
            await expect(
                stockToChain.connect(companyWallet).emergencyWithdraw(contractAddress)
        ...
        it("Should skip investor share transfer if investorShare == 0 in buyback", async function () {
            ...
            await expect(stockToChain.connect(companyWallet).buybackByCompanyBatch(0, 1, { value: buybackPrice + extra }))
                .to.emit(stockToChain, "BuybackExecuted");
        });
        ...
        it("Should revert on final batch if not enough tokens in contract to burn", async function () {
            ...
            const buybackPrice = await stockToChain.getBuybackPriceInPOL(amount);
            await expect(
                stockToChain.connect(companyWallet).buybackByCompanyBatch(0, 1, { value: buybackPrice + ethers.parseEther("0.1") })
            ).to.be.revertedWithCustomError(stockToChain, "NoTokensToBuyBack");
        });
        ...
        it("Should revert if eurUsdPrice from oracle is invalid", async function () {
            await eurUsdPriceFeed.updateAnswer(0);
            const amount = ethers.parseEther("1");
            await expect(stockToChain.getCurrentPriceInPOL(amount)).to.be.revertedWith("Invalid price feed data");
        });
    ...
    describe("Branch Coverage", function () {
        ...
        it("Should revert in final batch if contract holds insufficient tokens for burn", async function () {
            ...
            await stockToChain.connect(addr1).transfer(await stockToChain.getAddress(), half);
            ...
            await expect(
                stockToChain.connect(companyWallet).buybackByCompanyBatch(0, 1, { value: buybackPrice + extra })
            ).to.be.revertedWithCustomError(stockToChain, "NoTokensToBuyBack");
        });
        ...
        it("Should revert when startIndex equals endIndex in buybackByCompanyBatch", async function () {
            ...
            const buybackPrice = await stockToChain.getBuybackPriceInPOL(amount);
            await expect(
                stockToChain.connect(companyWallet).buybackByCompanyBatch(0, 0, { value: buybackPrice })
            ).to.be.revertedWith("Invalid index range");
        });
